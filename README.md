...
signed following this process:
  1. Signs the `pkgsign-manifest.json` file generated by Conan using `cosign`.
  2. The signature information is returned in the `sign()` method following this format
    ```json
    {
      ...
      "sign_artifacts": {
        ...
        "<signature-name2>": "<signature-file2>,
      }
    }
    ```
  3. If rekor is enabled, the signature of the package is registered against the rekor public log.
...
verified following this process:
  ...
  1. The checksums of the `pkgsign-manifest.json` are verified against the files in the package.
  2. The signature of the `pkgsign-signatures.json` file is verified using `cosign` and the public key
     associated to the provider defined in the signature information.
  3. If rekor is enabled, the signature of the package is also verified against the rekor public log.
